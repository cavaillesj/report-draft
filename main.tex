...
\todo[color = purple]{paragraph about disturbances/perturbations}
...
In practice, in some regions, fires are greatly affected by humans, wildfires have been considerably reduced due to the intervention of firefighters. Different management practices are used, depending notably on the country and the tree species, and mostly based on the misconception that lock the dynamics of a system (decreasing the variability)  will prevent it from collapse. In order to restore the natural dynamics of the forest, fires are sometimes allowed to run their course freely without intervention \citep{wallenius2011major}. 

%Finally, one of the problem of the study of fire is its stochastic aspect and the variability in both space and time, which add complexity to the modelling of fire events and dynamics \citep{agee1998landscape, lertzman1998three}.
...
\todo{catch" your reader and make them interested}

\todo{- In any event, this should be followed by more explicit description of what you did, and what the overall conclusions were. For instance, "We demonstrate that the dynamical behavior of the the forest-fire system can be understood along two axes of interest, allowing us to categorize seven distinct dynamical states of the system, which we call cases". Also, "We develop approximations the two stability measures of interest,...". And so on.}
...
\todo{talk more about possible values, or anything else that seems relevant}
\paragraph{} % parameter 
...
\paragraph{} \todo{ - The sentence "This upgrade is thus independent of the time study choose" is quite problematic. First, I don't see how this is an "upgrade". More importantly, it is not really independent, as there is an aspect of history here that is not trivial (that is, if you stop with high or low values of N and/or W, it matters, potentially quite a bit, to the collapse probability). Also, do you actually use this "collapse probability per time unit"? I think you should restructure this whole paragraph, starting by saying something like "We calculate the probability by using different simulations..."}
...
brief summary,  recall objective ...
...
Also, we only consider one kind of death wood, this can be details in several type (coarse woody debris, fine woody debris, below ground ...) \citep{russell_quantifying_2015}. In the literature, the data are rarely for all the wood, and, some wood burned easier than other. 

Moreover, in practice we can distinguish several fire regime (e.g., crown fires, severe surface fires, and light surface fires) \citep{reichle_fire_1981}. All of this different fires disturbed differently the dry wood. So the dynamic can be modelled in a more complex ways.

Less relevant, we only consider density biomass. However, the spatial distribution can affect fires propagation (especially for small fires). For example, a burned area can create an obstacle when another fire occur \citep{bergeron_natural_2002}.
...
\begin{itemize}
    \item stability ?
    \item collapse indicators
        \begin{itemize}
            \item usually, we have a "correlation" between measures
            \item it can be a problem to collapse without seeing "signal"
            \item numerically, bias when computing variability but, with "same fire" we have not a collapse (because of collapse) (we have already talk about this) 
            \item \citep{brock_variance_2006}
            \item \citep{carpenter2006rising}
            \item \citep{scheffer2015generic}
            \item \citep{dakos_robustness_2012}
            \item \citep{biggs_turning_2009}
        \end{itemize}
    \item management
        \begin{itemize}
            \item frequency : USA example (rocky mountains)
            \item $d$ fuel removals : lot of papers (fuel removal, very efficient to decrease cp)
            \begin{itemize}
                \item \citep{schoennagel_interaction_2004}
                \item \citep{agee_basic_2005}
                \item \citep{stephens_effects_2012}
                \item \citep{fernandes_prescribed_2013}
                \item mechanical thinning
                \begin{itemize}
                    \item \citep{liu_analyzing_2010} \citep{schoennagel_interaction_2004} \citep{wimberly_assessing_2009}
                \end{itemize}
            \end{itemize}
        \end{itemize}
    \item last words ...
